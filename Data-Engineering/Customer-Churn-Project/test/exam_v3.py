...
mean_bed = df.selectExpr("avg(bedroooms)").first()[0]
mode_loc = df.groupBy("location").count().orderBy("count", ascending=True).first()[0]
...
assembler = VectorAssembler(
    inputCols = ["area", "loc_idx", "bedrooms", "price_per_area"],
    outputCol="features"
)
...
pipeline=Pipeline([indexer, assembler, scaler, rf])
